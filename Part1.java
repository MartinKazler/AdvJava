...
public class Part1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("4. " + Arrays.stream(austin).filter(s -> s.length() > 8).distinct().count()
				+ " unique words more than 8 letters.");
		...
		System.out.println("7. a) Are all words shorter than 12 characters? ("
				+ (Arrays.stream(austin).filter(s -> s.length() > 12).count() > 0 ? "Yes" : "No")
				+ ")\n   b) Are all words longer than two letters? ("
				+ (Arrays.stream(austin).filter(s -> s.length() < 2).count() > 0 ? "Yes" : "No") + ")");
	}
